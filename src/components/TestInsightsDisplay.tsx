...
interface TestInsightsDisplayProps {
  ...
  groundTruthTexts?: Record<string, string>;
}

export const TestInsightsDisplay = ({
  insights,
  isLoading,
  error,
  selectedFileId,
  ...
  const groundTruthForSelected = selectedInsight
    ? groundTruthTexts?.[selectedInsight.fileName] ?? ""
    : "";

  const perFileExactness = useMemo(() => {
    if (!insights || !groundTruthTexts) return {};
    const map: Record<string, number> = {};
    for (const item of insights) {
      const gt = groundTruthTexts[item.fileName];
      if (gt && item.content) {
        map[item.id] = wordJaccard(item.content, gt);
      }
    }
    return map;
  }, [insights, groundTruthTexts]);
        ...
        {isLoading ? (
          ...
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                {selectedInsight && perFileExactness[selectedInsight.id] !== undefined && (
                  <div className="flex items-center gap-3 px-5 py-3 rounded-xl bg-primary/10 border border-primary/20 text-base">
                    <span className="text-muted-foreground font-medium">Exactness (Jaccard):</span>
                    <span className="font-bold text-primary text-xl">
                      {(perFileExactness[selectedInsight.id] * 100).toFixed(1)}%
                    </span>
                  </div>
                )}
              </div>
            ...
            <div className="hidden lg:block min-h-[600px]">
              <ResizablePanelGroup direction="horizontal">
                {/* Panel 1 - Currently Selected Image/Text */}
                ...
                <ResizablePanel defaultSize={34} minSize={20}>
                  <div className="h-full flex flex-col space-y-3 px-3">
                    <div className="flex items-center justify-between">
                      <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                        Extracted Content
                      </p>
                    ...
                    <div className="flex-1 p-4 rounded-lg border border-border/50 bg-background/50 min-h-[300px] overflow-y-auto">
                      {selectedInsight ? (
                        groundTruthForSelected ? (
                          ...
                          <div className="flex items-center justify-center h-full min-h-[200px]">
                            <p className="text-muted-foreground text-sm text-center">
                              No ground truth available for this file.
                            </p>
                          </div>
                        )
                      ) : (
                        <div className="flex items-center justify-center h-full min-h-[200px]">
                          <p className="text-muted-foreground text-sm text-center">
                            Select a file to view its ground truth.
                          </p>
                        </div>
                      )}
                    </div>
          ...
          <div className="flex flex-col items-center justify-center h-full text-center min-h-[200px]">
            ...
            <p className="text-muted-foreground/60 text-xs mt-2">
              Select a use case and click "Extract Insights" to get started.
            </p>
          </div>
        )}
      </div>
