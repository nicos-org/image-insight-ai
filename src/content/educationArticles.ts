...
Simply put:
- ChatGPT-like tools are already entering GMP workflows.
- They can improve speed and access to information, but they can also create compliance risk.
- Inspectors should evaluate implementation quality, controls, and traceability.
...
accuracy, consistency, and auditability of outputs. **Areas where naive use of LMs could falter include a fundamental issue: 
that large language models are probabilistic, not deterministic.** 

### Perceived Non-Determinism in Practice - "every time different answers"
...
outputs. This is a benchmark that typical free-form AI might not meet. Now this is not entirely true. This is 
...
In short:
- Records must remain attributable, with clear ownership of who asked, reviewed, and acted.
- AI outputs must be traceable to approved data sources and decision paths.
- Audit trails must capture prompts, responses, timestamps, and user context.
- Hallucination risk must be controlled through validation, oversight, and source verification.

> [!WARNING]
> Regulatory expectation: If a company deploys a ChatGPT-like tool in a GMP process, they should treat it as a GxP computerized system. That means performing risk assessments, validation (IQ/OQ/PQ), and putting controls in place to ensure it operates reliably within its intended use.

In fact, regulators have explicitly started scrutinizing use of AI. The FDA, EMA, MHRA and others expect firms to demonstrate that 
...
From an operational standpoint, this could offer real benefits. The operator can quickly confirm if a cycle was 
within spec without manually cross-checking each data point. They can get guidance on procedures without thumbing 
through a manual, which might reduce human error (no more "I couldn't find the info, so I guessed" situations). It 
can also speed up troubleshooting. For example, asking "What does error code 7 mean?" could immediately pull up the 
relevant manual section that says error 7 is a pressure sensor fault and suggests checking the seal. In a busy 
manufacturing setting, saving minutes on each query and ensuring accurate info is retrieved can add up to efficiency gains.

However, from a compliance and inspection perspective, this scenario raises important questions. The chatbot is 
effectively interpreting GMP data and controlled documents on the fly. We need to ensure it's doing that correctly 
and safely:
...
- **Scope of the AI's Knowledge:** Is the AI strictly limited to the autoclave's data and documents? Ideally, yes, and we want a narrow, domain-specific AI assistant. A worrying scenario would be if the chatbot also has general internet or broad knowledge. For instance, if it's connected to a general AI model that wasn't fine-tuned, it might start giving advice beyond the local documentation (e.g. "I recall a case where someone used a different cycle profile..." based on training data from elsewhere). That could be dangerous or simply non-applicable. A good implementation keeps the AI focused on the task-specific content.
...
- **Inconsistent Outputs (Non-determinism):** You might notice that the system's answers vary widely. Today it says one thing, tomorrow something slightly different to the same query. This variability might confuse operators and will certainly alarm inspectors. It indicates the AI is not configured for consistency. In the worst cases, it could even contradict itself, for example, two repetitions of a query about whether a cycle was within spec yield two slightly different summaries, or perhaps one time it omits a detail it mentioned the first time. Such inconsistency undermines trust and traceability, and as discussed, it violates the expectation of reproducibility for electronic records.
